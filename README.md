...
**The *cydia* branch of XPrivacy works, but I can't get [Cydia Substrate](http://www.cydiasubstrate.com/) to load XPrivacy if Xposed is disabled (it works with Xposed enabled and XPrivacy disabled). Furthermore, Cydia Substrate is not able to hook *Settings.Secure*, for which a boot class loader is needed. Also *Build.SERIAL* cannot be restricted. Everything else seems to work okay.**
